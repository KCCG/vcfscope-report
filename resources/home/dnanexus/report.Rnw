...
# ggplot(marginalizePerformance(class_subsets.performance_thresholded, quote(muttype == "Ins" & zyg %in% c("RA", "AA")), .(zyg, mutsize, depth)), aes(x = depth, y = sens, group = zyg, colour = zyg)) + 
#     geom_line() + 
#     geom_ribbon(aes(ymin = sens.lci, ymax = sens.uci, fill = zyg), alpha = 0.25, colour = NA) + 
#     geom_point(size = 2, shape = 21, fill = "white") + 
#     labs(x = "Number of bases inserted (inclusive)", y = "Sensitivity", fill = "Zygosity", colour = "Zygosity", title = "Insertion Detection Performance") + 
...
# ggplot(marginalizePerformance(class_subsets.performance_thresholded, quote(muttype == "Del" & zyg %in% c("RA", "AA")), .(zyg, mutsize, depth)), aes(x = depth, y = sens, group = zyg, colour = zyg)) + 
#     geom_line() + 
#     geom_ribbon(aes(ymin = sens.lci, ymax = sens.uci, fill = zyg), alpha = 0.25, colour = NA) + 
#     geom_point(size = 2, shape = 21, fill = "white") + 
