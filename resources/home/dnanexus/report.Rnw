...
temp.sample_table = data.frame(
    "Sample Label" = sapply(data, function(x) texquote(x$label)),
...
Averaged across all samples and depths, the aggregate false positive rate was \Sexpr{formatC(mean(colSums(fpr.NFPi))/temp.size_gold*1e6, format = "f", digits = 2)} (range $\Sexpr{formatC(min(colSums(fpr.NFPi))/temp.size_gold*1e6, format = "f", digits = 2)}-\Sexpr{formatC(max(colSums(fpr.NFPi))/temp.size_gold*1e6, format = "f", digits = 2)}$) false positive calls per megabase.
