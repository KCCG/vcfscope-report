...
colnames(temp) = c("Sample label", "False positive calls", "Bases analysed", "FPR estimate (false positive calls per Mb)")
saved_results$tables$fpr.sample = temp

print(xtable(temp, caption = "Overall false positive rate (false positives per megabase)"), sanitize.text.function = function(x) x, include.rownames = FALSE, hline.after = c(-1, 0), tabular.environment = "longtable", caption.placement = "top")
@
