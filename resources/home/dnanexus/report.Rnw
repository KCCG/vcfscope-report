...
<<sensitivity-plots-snv,fig.width=5,fig.height=5,out.width='2.5in',fig.show='hold'>>=
...
ggplot(substs_vs_depth.melted, aes(x = depth, y = count, lty = sample, shape = sample, colour = zyg, group = sample:zyg)) + 
    geom_line() + geom_point(size = 2, fill = "white") + 
    labs(x = "Sequence depth (inclusive)", y = "Variants with this depth", title = "SNV Counts vs Depth", colour = "Zygosity", shape = "Sample", lty = "Sample Label") + 
    theme_bw()
@
...
<<sensitivity-plots-ins,fig.width=5,fig.height=5,out.width='2.5in',fig.show='hold'>>=
...
<<sensitivity-plots-del,fig.width=5,fig.height=5,out.width='2.5in',fig.show='hold'>>=
...
\item Software: \begin{itemize}
	...
	\item Genome: \texttt{\Sexpr{texquote(shared_params$genome)}} (\texttt{\Sexpr{shared_params$genome_version}})
